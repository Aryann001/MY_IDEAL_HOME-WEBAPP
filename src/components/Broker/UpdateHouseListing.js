...
export default function UpdateHouseListing() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { updateHouseId } = useParams();
  ...
  const {
    ...
    error: hdError,
    house,
  } = useSelector((state) => state.houseDetails);
  ...
  const bannerSubmitHandler = (e) => {
    ...
    dispatch(updateHouse(updateHouseId, bannerData));
  };

  useRunOnce({
    fn: () => {
      dispatch(houseDetails(updateHouseId));
    },
  });
  ...
  useEffect(() => {
    ...
    if (!hdLoading) {
      ...
      setPincode(house.pincode);
      setPrice(house.price);
      setDiscountedPrice(house.discountedPrice);
      setBedrooms(house.bedrooms);
      setBathrooms(house.bathrooms);
      setCategory(house.type);
      ...
      if (house.images !== undefined) {
        ...
        for (let i = 0; i < house.images.length; i++) {
          bimgArr.push(house.images[i].url);
        }
  ...
  }, [dispatch, navigate, error, isUpdated, house, hdError, hdLoading]);
